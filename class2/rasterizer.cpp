...
static bool insideTriangle(int x, int y, const Vector3f *_v) {
    ...
    // _v[2] 0 means negative, 1 means positive
    int flag = -1;

    for(int i = 0; i < 3; i++) {
        // the current point
        Eigen::Vector3f p0 = {float(x), float(y), 0};
        // the 1st vertex
...
void rst::rasterizer::rasterize_triangle(const Triangle &t) {
    ...
    int x_s = std::min(v[0].x(), std::min(v[1].x(), v[2].x()));
    int x_e = std::max(v[0].x(), std::max(v[1].x(), v[2].x()));

    int y_s = std::min(v[0].y(), std::min(v[1].y(), v[2].y()));
